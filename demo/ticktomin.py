...
class MainWindows(QMainWindow):
    def __init__(self):
        ...
        curve=self.draw1.plot(pen='y')
        tmpline=mindf.close.cumsum()
        print(tmpline.head())
        tmpline = tmpline.apply(lambda x: x/(tmpline[tmpline==x].index[0]+1))
        curve.setData(tmpline)
        self.l.nextRow()
