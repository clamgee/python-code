...
class KLineWidget(KeyWraper):
    ...
    def refresh(self):
        # 更新X軸邊界
        x=int(len(self.candle.pictures))        
        minutes = int(self.countK/2)
        xmin    = max(0,x-minutes)
        try:
            xmax    = x
        except:
            xmax    = x
        # self.pwOI.setRange(xRange = (xmin,xmax))
        ymin=self.candle.data.loc[xmin:xmax,['low']].values.min()
        ymax=self.candle.data.loc[xmin:xmax,['high']].values.max()
        # print('xmin,xmax,ymin,ymax= ',xmin,xmax,ymin,ymax)
        self.pwKL.setRange(xRange = (xmin,xmax),yRange = (ymin,ymax))
