# 先把API com元件初始化
import os
import ctypes #RequestLiveTick會用到

# 第二種讓群益API元件可導入Python code內用的物件宣告，需要先註冊 SKCOM.dll
...
class Quote(Frame):
    ...
    def btnQueryStocks_Click(self):
        try:
            ...
            messagebox.showerror("btnQueryStocks error！",e)
...
class SKQuoteLibEvents:
    ...
    def OnNotifyTicks(self,sMarketNo,sIndex,nPtr,nTimehms,nTimemillismicros,nBid,nAsk,nClose,nQty,nSimulate):
        strMsg ='時間:',nTimehms,nTimemillismicros,'價格:',int(nClose/100),'成交量:',nQty
        WriteMessage(strMsg,Gobal_Quote_ListInformation)
